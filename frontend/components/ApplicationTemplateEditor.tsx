...
const ApplicationTemplateEditor: React.FC = () => {
  ...
  const { data: template, isLoading } = useApplicationTemplateDetail(appId || '', templateId || '');
  const updateMutation = useUpdateApplicationTemplate(appId || '');
  ...
  const getPreviewHtml = () => {
    ...
    const mockValues: Record<string, string> = {
      ...
      '{{os}}': 'Windows 11',
      '{{application_name}}': template.name || 'My Application',
...
export default ApplicationTemplateEditor;
