...
func (s *TemplateService) CreateEmailTemplate(ctx context.Context, req *models.CreateEmailTemplateRequest, createdBy uuid.UUID) (*models.EmailTemplate, error) {
	...
	// Check uniqueness: one template per type (global)
	if req.Type != models.EmailTemplateTypeCustom {
		existing, _ := s.repo.GetEmailTemplateByType(ctx, req.Type)
		if existing != nil {
			return nil, fmt.Errorf("global template of type '%s' already exists", req.Type)
		}
	}
...
func (s *TemplateService) CreateEmailTemplateForApp(ctx context.Context, applicationID uuid.UUID, req *models.CreateEmailTemplateRequest, createdBy uuid.UUID) (*models.EmailTemplate, error) {
	...
	// Check uniqueness: one template per type per application
	if req.Type != models.EmailTemplateTypeCustom {
		existing, _ := s.repo.GetEmailTemplateByTypeAndApp(ctx, req.Type, applicationID)
		if existing != nil {
			return nil, fmt.Errorf("template of type '%s' already exists for this application", req.Type)
		}
	}
...
func (s *TemplateService) InitializeTemplatesForApp(ctx context.Context, applicationID uuid.UUID, createdBy uuid.UUID) error {
	...
	for _, templateType := range templateTypes {
		...
		if err := s.repo.UpsertEmailTemplate(ctx, emailTemplate); err != nil {
			return fmt.Errorf("failed to upsert %s template: %w", templateType, err)
		}
	}
